    ...
    <div className="grid gap-3 lg:grid-cols-[420px_1fr]">
      <section className="grid gap-3">
        <div className="rounded-2xl border border-white/[0.08] bg-slate-900/72 p-3 shadow-glow backdrop-blur-xl">
          <h2 className="mb-3 text-lg font-black text-white">OBS WebSocket</h2>
          <div className="grid gap-3">
            <Field label="URL"><Input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="ws://127.0.0.1:4455" /></Field>
            <Field label="Password"><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></Field>
            <div className="grid grid-cols-2 gap-2">
              <Button variant="primary" icon={<PlugZap className="h-4 w-4" />} onClick={connect}>{obs.connecting ? "Connecting..." : "Connect"}</Button>
              <Button onClick={obs.disconnect}>Disconnect</Button>
            </div>
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-slate-900/72 p-3 shadow-glow backdrop-blur-xl">
          <h2 className="mb-3 text-lg font-black text-white">Import / Export</h2>
          <div className="grid gap-2">
            <Button icon={<Download className="h-4 w-4" />} onClick={exportConfig}>Export config JSON</Button>
            <a ref={downloadRef} className="hidden" />
            <Textarea value={importText} onChange={(event) => setImportText(event.target.value)} placeholder="Paste exported JSON here" />
            <Button icon={<Upload className="h-4 w-4" />} onClick={importConfig}>Import config JSON</Button>
            <Button variant="warning" icon={<RotateCcw className="h-4 w-4" />} onClick={() => resetConfig().then(() => addLog("warning", "Config reset to sample"))}>Reset sample config</Button>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-3">
        <div className="rounded-2xl border border-white/[0.08] bg-slate-900/72 p-3 shadow-glow backdrop-blur-xl">
          <h2 className="mb-3 text-lg font-black text-white">Validation</h2>
          <div className="grid gap-2">
            {issues.length ? issues.map((issue) => (
              <div key={issue.message} className={`rounded-xl border px-3 py-2 text-sm font-semibold ${issue.level === "error" ? "border-rose-400/40 bg-rose-500/10 text-rose-100" : "border-amber-400/40 bg-amber-500/10 text-amber-100"}`}>{issue.message}</div>
            )) : <div className="rounded-xl border border-emerald-400/40 bg-emerald-500/10 px-3 py-2 text-sm font-semibold text-emerald-100">No validation issues found.</div>}
          </div>
        </div>

        <div className="rounded-2xl border border-white/[0.08] bg-slate-900/72 p-3 shadow-glow backdrop-blur-xl">
          <h2 className="mb-3 text-lg font-black text-white">Log</h2>
